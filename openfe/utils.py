...
def formula_to_tree(string):
    ...
    def find_prev(string):
        if string[-1] != ")":
            return max(
                [(0 if is_trivial_char(c) else i + 1) for i, c in enumerate(string)]
            )
        level, pos = 0, -1
    ...
    if string[0] == "(":
        return Node(
            string[p2 - 1],
            [formula_to_tree(string[p1 : p2 - 1]), formula_to_tree(string[p2:-1])],
        )
    else:
        return Node(
            string[: p1 - 1],
...
def _cal(feature, n_train):
    try:
        ...
        _data = pd.read_feather(
            "./openfe_tmp_data.feather", columns=base_features
...
def transform(X_train, X_test, new_features_list, n_jobs, name=""):
    ...
    with tqdm.tqdm(
        ...
        for i, feature in enumerate(new_features_list):
            progress_bar.set_postfix(feature=tree_to_formula(feature))
            results.append(ex.submit(_cal, feature, n_train))
...
def rename_columns(df):
    return df.rename(
        columns={col: ("autoFE-%d" % i) for i, col in enumerate(df.columns)}
    )
