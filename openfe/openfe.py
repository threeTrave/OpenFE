            ...
            for f2 in current_order_num_features[k:] + lower_order_num_features:
                if check_xor(f1, f2):
                    num_candidate_features.append(
                        Node(op, children=[deepcopy(f1), deepcopy(f2)])
                    )
        ...
        for f in current_order_num_features:
            for cat_f in current_order_cat_features + lower_order_cat_features:
                if check_xor(f, cat_f):
                    num_candidate_features.append(
                        Node(op, children=[deepcopy(f), deepcopy(cat_f)])
                    )
...
def get_candidate_features(
    ...
    if ordinal_features is None:
        ordinal_features = []
    assert (
        len(set(numerical_features) & set(categorical_features) & set(ordinal_features))
        == 0
    )
    num_features = []
    cat_features = []
    ...
    while order > 0:
        ...
        lower_order_num_features, lower_order_cat_features = (
            current_order_num_features,
            current_order_cat_features,
        )
...
class OpenFE:
    ...
    def fit(
        ...
        self.candidate_features_list = self.get_candidate_features(
            candidate_features_list
        )
        ...
        self.myprint(
            f"The number of candidate features is {len(self.candidate_features_list)}"
        )
        self.myprint("Start stage I selection.")
        self.candidate_features_list = self.stage1_select()
        self.myprint(
            f"The number of remaining candidate features is {len(self.candidate_features_list)}"
        )
        self.myprint("Start stage II selection.")
        self.new_features_scores_list = self.stage2_select()
        self.new_features_list = [
            feature for feature, _ in self.new_features_scores_list
        ]
    ...
    def process_label(self):
        if self.task == "regression":
            ...
            self.label[self.label.columns[0]] = (
                self.label[self.label.columns[0]].astype("category").cat.codes
            )
    ...
    def get_index(self, train_index, val_index):
        if train_index is None or val_index is None:
            if self.task == "classification":
                ...
                _, _, train_y, test_y = train_test_split(
                    self.data, self.label, test_size=0.2, random_state=self.seed
                )
    ...
    def data_to_dataframe(self):
        try:
            if not isinstance(self.data, pd.DataFrame) or not isinstance(
                ...
                warnings.warn(
                    "data and label should both be pd.DataFrame and have the same index!!!"
                )
            ...
            raise ValueError(
                f"Cannot transform data and label into dataframe due to error: {e}"
            )
    ...
    def get_init_score(self, init_scores, use_train=False):
        if init_scores is None:
            ...
            if self.feature_boosting:
                ...
                if (
                    self.task == "classification"
                    and label[label.columns[0]].nunique() > 2
                ):
                    init_scores = np.zeros(
                        (len(data), label[label.columns[0]].nunique())
                    )
                else:
                ...
                for train_index, val_index in skf.split(data, label):
                    ...
                    if use_train:
                        init_scores[train_index] += (
                            gbm.predict_proba(X_train, raw_score=True)
                            if self.task == "classification"
                            else gbm.predict(X_train)
                        ) / (skf.n_splits - 1)
                    else:
                        init_scores[val_index] = (
                            gbm.predict_proba(X_val, raw_score=True)
                            if self.task == "classification"
                            else gbm.predict(X_val)
                        )
                ...
                if self.task == "regression":
                    init_scores = np.array(
                        [np.mean(self.label.values.ravel())] * len(self.label)
                    )
                    ...
                    init_scores = np.array(
                        [logit(np.mean(self.label.values.ravel()))] * len(self.label)
                    )
    ...
    def check_init_scores(self, init_scores):
        if self.task == "classification":
            if (
                (init_scores[:100].values >= 0) & (init_scores[:100].values <= 1)
            ).all():
                warnings.warn(
                    "The init_scores for classification should be raw scores instead of probability."
    ...
    def stage1_select(self, ratio=0.5):
        ...
        results = self._calculate_and_evaluate(
            self.candidate_features_list, train_idx, val_idx
        )
        ...
        while idx != len(train_index_samples):
            ...
            results = self._calculate_and_evaluate(
                candidate_features_list, train_idx, val_idx
            )
            candidate_features_scores = sorted(
    ...
    def _evaluate(
        self, candidate_feature, train_y, val_y, train_init, val_init, init_metric
    ):
        try:
            ...
            if self.stage1_metric == "predictive":
                ...
                if self.task == "regression":
                    r = mutual_info_regression(
                        pd.concat([train_x, val_x], axis=0)
                ...
                raise NotImplementedError(
                    "Cannot recognize filter_metric %s." % self.stage1_metric
                )
            return score
        except:
    ...
    def _calculate_and_evaluate_multiprocess(
        self, candidate_features, train_idx, val_idx
    ):
        try:
            results = []
            base_features = {"openfe_index"}
            ...
            for candidate_feature in candidate_features:
                ...
                score = self._evaluate(
                    candidate_feature, train_y, val_y, train_init, val_init, init_metric
                )
    ...
    def _trans(self, feature, n_train):
        try:
            ...
            if (str(feature.data.dtype) == "category") | (
                str(feature.data.dtype) == "object"
            ):
                pass
            else:
        ...
        return (
            (
                (str(feature.data.dtype) == "category")
    ...
    def transform(self, X_train, X_test, new_features_list, n_jobs, name=""):
        ...
        for i, res in enumerate(results):
            ...
            _test.append(d2)
            if is_cat:
                cat_feats.append("autoFE_f_%d" % i + name)
